...
class SeriesAccessor:
    ...
    def check(self, random_state=None):
        results = {}
        if is_continuous(self._obj):
...
class DataFrameAccessor:
    ...
    def imbalance_degree(self, target=None):
        if is_continuous(self._obj):
            warnings.warn('The column does not seem categorical.')
    ...
    def dummy_scores(self, target=None, task=None, random_state=None):
        if task is None:
            task = 'regression' if is_continuous(self._obj[target]) else 'classification'

        if task == 'classification':
            scores = dummy_classification_scores(self._obj[target], random_state=random_state)
        elif task == 'regression':
            scores = dummy_regression_scores(self._obj[target])
        else:
            raise ValueError("`task` must be 'classification' or 'regression', or None to decide automatically.")
